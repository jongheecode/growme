...
> 2026-07-23 기준, 코드베이스를 직접 읽어 작성한 현재 상태 스냅샷. 이후 기능이 추가/변경되면 이 문서도 같이 갱신해야 함.
...
- 구분선("또는") 아래 **구글로 계속하기**(흰 배경 아웃라인 버튼), **카카오로 계속하기**(`#FEE500` 배경) 버튼.
- 하단에 "회원가입" 전환 링크.
...
- 실제 이메일 발송 인프라가 없어 재설정 링크(토큰 포함)는 **서버 콘솔 로그**로만 나가고, 사용자가 토큰을 직접 입력해야 함(딥링크 자동 처리 아님).
...
- 중앙에 보유 포인트 배지 + 탭 가능한 꾸미 캐릭터(탭하면 KkumiInfoModal 오픈).
- 우하단 플로팅 "오늘의 미션" 시트(`TaskSheet`, 접힌 상태는 `완료수/전체` 원형 버튼):
  - 할일 목록: 완료(초록 체크)/대기(빈 원, 탭하면 완료 처리)/실패(빨간 X) 상태 표시, 카테고리 배지 + 난이도 + XP 배지.
...
- 할일 탭 → `MissionModal` 바텀시트: 집중 타이머(시작/멈추기, 30초 간격 하트비트 전송) + 완료/포기 버튼.
- 완료 시 AI가 생성한 리액션 텍스트 + XP/포인트 보상을 `ReactionModal`로 보여줌. 이번 완료로 **첫 부화**(알→종 확정)가 일어나면 알이 흔들리는 애니메이션(1.5초) 후 종/단계를 리빌하는 2단계 연출.
- 실패(기한 초과)로 전환된 할일의 리액션은 다음 목록 조회 시 큐에 쌓여 하나씩 순차적으로 보여줌(즉시 완료 리액션과 별개 큐).
...
- 메뉴 그리드(2열): 친구 / 랭킹 / 챌린지.
- **오늘 미션 리마인더 토글**: 매일 20:00 로컬 알림 예약/취소, 권한 거부 시 안내 문구.
- **계정 설정** 진입 카드.
...
- 초대코드 표시(공유용).
- 생성자가 아니면 "나가기" 버튼(생성자는 못 나감 — 챌린지를 지우는 기능 자체가 없음).
...
- 완료 시 `GrowthProfile.points`가 XP만큼 증가하고, AI가 완료/실패 각각에 대해 개인화된 리액션 문장을 생성(사용자의 "성격 유형" 반영, best-effort — 실패해도 완료 처리 자체는 롤백되지 않음).
...
- **성격 유형**: 완료/실패 이력이 3건 이상 쌓이면 계산. 축A(완료율 70%↑=STEADY / 미만=LOOSE) × 축B(기한의 절반 이전 완료 비율 50%↑=EASYGOING / 미만=LASTMINUTE) → 4유형(산책가형/질주러형/몽상가형/벼락치기형).
...
- 친구 요청은 닉네임 검색 기반(고유 식별자 공유 없이 닉네임 텍스트로 매칭 — 닉네임 중복 가능성에 대한 처리는 없음, `findFirst`로 첫 매치만 사용).
- 랭킹은 "전체"(모든 유저의 완료 XP 합, 상위 50명) / "친구"(본인+수락된 친구만) × "주간"(최근 7일 완료분) / "전체기간".
- 챌린지는 기간 내 완료 XP 합으로 진행률 계산(카테고리 지정 시 해당 카테고리만 집계), 초대코드로 참여, 생성자는 탈퇴 불가.
...
- **비밀번호 재설정**: SHA-256 해시된 1회용 토큰(1시간 만료), 실제 이메일 발송 없이 서버 콘솔 로그로만 링크 노출(운영 전환 시 메일 발송 서비스 연동 필요 — `backend/src/services/mailer.ts`에 인터페이스만 있음).
- **구글/카카오 로그인**: 네이티브 SDK 대신 `expo-auth-session` 브라우저 기반 OAuth(구글은 `id_token`+nonce, 카카오는 `access_token` 암묵적 플로우). 카카오 네이티브 SDK는 Expo Go에서 로드 불가능해서 의도적으로 회피. **실제 로그인 자체는 Google Cloud Console/Kakao Developers에 앱을 등록하고 발급받은 client ID를 `mobile/app.json`의 `extra.googleClientId`/`kakaoClientId`, 백엔드 `.env`의 `GOOGLE_CLIENT_ID`/`KAKAO_REST_API_KEY`에 채워야 동작함 — 현재는 빈 값.**
- **레이트리밋**: `/api/auth/*`(로그인/회원가입/비밀번호 재설정/구글·카카오 로그인)에 15분당 20회 제한.

### 5.9 알림

- 로컬 전용(서버 푸시 아님, 디바이스 토큰 저장/발송 인프라 없음): 매일 20:00에 "꾸미가 기다리고 있어요" 알림 1건 예약. 리마인더 시간이 완료 여부와 무관하게 고정 발송됨(오늘 미션을 이미 다 끝냈어도 옴).

### 5.10 오프라인 표시

- 앱 전역(`App.tsx` 루트)에 `OfflineBanner`가 항상 마운트되어 있고, 기기가 오프라인이거나(연결은 됐지만) 인터넷이 안 잡힐 때 화면 최상단에 빨간 배너로 알림. 화면별 에러 메시지는 여전히 "다시 시도"만 있는 채로 남아있고, 오프라인 여부를 구분해 문구를 바꾸는 화면은 아직 없음.
...
- **AI 기능 전체(목표 챗, 태스크 추천, 완료/실패 리액션)가 Anthropic API 크레딧에 의존** — 크레딧이 없으면 관련 기능이 500 에러로 실패.
- 구글/카카오 로그인은 코드는 완성됐지만 **실제 client ID 미설정**으로 지금은 눌러도 동작 안 함.
- 비밀번호 재설정 메일이 **실제 발송되지 않음**(콘솔 로그만).
- 이메일 인증 절차 없음(가입 즉시 사용 가능, 이메일 소유 확인 안 함).
- 신고 기능은 **접수만 되고 검토/처리 UI 없음**.
- 알림은 로컬 1건(매일 20시 고정)뿐 — 서버 발 푸시, 사용자 지정 시간, 다중 알림 없음.
- 챌린지는 생성자가 삭제/종료하는 기능이 없음(멤버 탈퇴만 가능).
- 닉네임 중복 검사 없음(친구 찾기가 닉네임 첫 매치 기준이라 동명이인 시 혼선 가능).
- EAS Build 설정은 준비돼 있지만 실제 빌드는 트리거된 적 없음(Expo 계정 로그인 필요).
- 에러 트래킹/모니터링(Sentry 등) 미연동.
- 화면별 오프라인/서버오류 구분 메시지 없음(전역 오프라인 배너만 있음).
